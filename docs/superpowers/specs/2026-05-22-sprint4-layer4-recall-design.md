...
**lock**: `~/.claude/mindvault-v2/memory-indexer.lock` (flock LOCK_NB, 동시 실행 차단).
...
`~/.claude/mindvault-v2/debug.log`에 prefix로 출처 구분:
```
...
# DB 상태
sqlite3 ~/.claude/mindvault-v2/index.db "SELECT COUNT(*) FROM memories; SELECT COUNT(*) FROM memories_vec;"

# 최근 hook 로그
tail -20 ~/.claude/mindvault-v2/debug.log | grep hook-recall
```
...
**선결조건**: BGE-M3 서버 기동. `MV2_SKIP_INTEGRATION=1`이면 skip.
...
MV2_SKIP_INTEGRATION=0 python3 -m unittest tests.test_integration
