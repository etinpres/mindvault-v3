...
**Spec:** `~/my-folder/apps/mindvault-v2/docs/superpowers/specs/2026-05-22-sprint4-layer4-recall-design.md`
...
**Files:**
- Create: `~/my-folder/apps/mindvault-v2/tests/fixtures/memory/feedback_test_mail.md`
- Create: `~/my-folder/apps/mindvault-v2/tests/fixtures/memory/project_test_scanner.md`
- Create: `~/my-folder/apps/mindvault-v2/tests/fixtures/memory/feedback_test_html.md`
- Modify: `~/my-folder/apps/mindvault-v2/pyproject.toml` 또는 의존성 명시 파일

- [ ] **Step 0.1: Working directory 확인**

```bash
cd ~/my-folder/apps/mindvault-v2
pwd
ls handoff/ src/ tests/
```
Expected: `/Users/yonghaekim/my-folder/apps/mindvault-v2`, 기존 Sprint 1~3 산출물 존재.
...
```bash
cat > ~/my-folder/apps/mindvault-v2/.gitignore <<'EOF'
__pycache__/
*.pyc
*.pyo
.pytest_cache/
...
**Files:**
- Create: `~/my-folder/apps/mindvault-v2/plist/com.yonghaekim.bge-m3-mlx.plist`
- Create: `~/my-folder/apps/mindvault-v2/scripts/bge_m3_server.py`
...
```bash
python3 ~/my-folder/apps/mindvault-v2/scripts/bge_m3_server.py &
SERVER_PID=$!
sleep 5  # 모델 로딩 시간
...
<dict>
    ...
    <array>
        ...
        <string>/Users/yonghaekim/my-folder/apps/mindvault-v2/scripts/bge_m3_server.py</string>
    </array>
    <key>RunAtLoad</key>
    <true/>
    <key>KeepAlive</key>
    ...
    <string>/Users/yonghaekim/.claude/mindvault-v2/bge_m3.log</string>
    <key>StandardErrorPath</key>
    <string>/Users/yonghaekim/.claude/mindvault-v2/bge_m3.err</string>
</dict>
</plist>
```
...
```bash
mkdir -p ~/.claude/mindvault-v2
cp ~/my-folder/apps/mindvault-v2/plist/com.yonghaekim.bge-m3-mlx.plist ~/Library/LaunchAgents/
...
**Files:**
- Modify: `~/my-folder/apps/mindvault-v2/src/indexer.py` (SCHEMA_VERSION 1 → 2, memories_* 테이블 추가)
- Test: `~/my-folder/apps/mindvault-v2/tests/test_schema_v2.py`
...
[ -f ~/.claude/mindvault-v2/index.db ] && cp ~/.claude/mindvault-v2/index.db ~/.claude/mindvault-v2/index.db.v1.bak

# open_db로 자동 마이그레이션 트리거
python3 -c "
import sys
sys.path.insert(0, '/Users/yonghaekim/my-folder/apps/mindvault-v2/src')
from indexer import open_db
from pathlib import Path
conn = open_db(Path('/Users/yonghaekim/.claude/mindvault-v2/index.db'))
...
python3 -c "
import sqlite3
c = sqlite3.connect('/Users/yonghaekim/.claude/mindvault-v2/index.db')
...
**Files:**
- Create: `~/my-folder/apps/mindvault-v2/src/memory_indexer.py`
- Test: `~/my-folder/apps/mindvault-v2/tests/test_memory_indexer.py`
...
**Files:**
- Create: `~/my-folder/apps/mindvault-v2/src/memory_search.py`
- Test: `~/my-folder/apps/mindvault-v2/tests/test_memory_search.py`
...
**Files:**
- Create: `~/my-folder/apps/mindvault-v2/hooks/memory-recall.py` (배포본 → `~/.claude/hooks/`)
- Test: `~/my-folder/apps/mindvault-v2/tests/test_memory_hook.py`
...
```bash
chmod +x ~/my-folder/apps/mindvault-v2/hooks/memory-recall.py
```
...
```bash
echo '{"prompt":"이메일 보내는 방법"}' | python3 ~/my-folder/apps/mindvault-v2/hooks/memory-recall.py
```
...
**Files:**
- Modify: `~/my-folder/apps/mindvault-v2/src/recall_cli.py`
- Test: `~/my-folder/apps/mindvault-v2/tests/test_recall_cli.py`
...
```bash
python3 ~/my-folder/apps/mindvault-v2/src/recall_cli.py "메일" --source memory | python3 -m json.tool | head -30
```
Expected: memory 결과 JSON. (BGE-M3 떠있고 인덱스 있어야 의미 있음)

- [ ] **Step 6.6: /recall 스킬 markdown 업데이트**

`~/my-folder/apps/mindvault-v2/skill/recall.md`의 명령 부분을 양쪽 처리하도록 보강:
```bash
...
**Files:**
- Modify: `~/my-folder/apps/mindvault-v2/src/memory_review_cli.py` (approve 후 incremental_index 1줄)
- Modify: `~/my-folder/apps/mindvault-v2/install.sh`
- Modify: `~/my-folder/apps/mindvault-v2/uninstall.sh`
- Test: `~/my-folder/apps/mindvault-v2/tests/test_install_uninstall.py`
...
class TestApproveReindex(unittest.TestCase):
    @unittest.skipIf(
        __import__("os").environ.get("MV2_SKIP_INTEGRATION") == "1",
        "skip integration",
    )
...
# 4) memories 테이블 확인
sqlite3 ~/.claude/mindvault-v2/index.db "SELECT name FROM memories WHERE name='sprint4-test'"
```
...
```bash
cat ~/my-folder/apps/mindvault-v2/install.sh
```
...
```bash
shellcheck ~/my-folder/apps/mindvault-v2/install.sh 2>&1 | head -20 || echo "shellcheck not installed - skip"
bash -n ~/my-folder/apps/mindvault-v2/install.sh && echo "syntax OK"
```
Expected: `syntax OK`.
...
**Files:**
- Create: `~/my-folder/apps/mindvault-v2/tests/test_integration.py`
- Create: `~/my-folder/apps/mindvault-v2/tests/benchmark_search.py`

이 task는 BGE-M3 서버 + 실제 DB 필요. CI에서는 `MV2_SKIP_INTEGRATION=1`로 skip.
...
# 통합 테스트
cd ~/my-folder/apps/mindvault-v2
MV2_SKIP_INTEGRATION=0 python3 -m unittest tests.test_integration -v 2>&1 | tail -20
```
...
```bash
python3 ~/my-folder/apps/mindvault-v2/tests/benchmark_search.py
```
...
**Files:**
- Modify: `~/my-folder/apps/mindvault-v2/README.md`
- Modify: `~/my-folder/apps/mindvault-v2/handoff/SESSION-CHECKPOINT.md`
...
sqlite3 ~/.claude/mindvault-v2/index.db "SELECT COUNT(*) FROM memories, memories_fts, memories_vec"
```
...
```bash
# debug log 확인
tail -30 ~/.claude/mindvault-v2/debug.log | grep -E "hook-recall|mem-indexer"
```
