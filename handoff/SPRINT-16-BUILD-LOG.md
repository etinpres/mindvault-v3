...
- **Multi-source 영구 등록** (`src/sources_cli.py`): `~/.claude/mindvault-v2/sources.json`
  config file 로 추가 indexing dir 영구 저장. Sprint 11 의 env var (MV2_EXTRA_MEMORY_DIRS) 와
...
config: `~/.claude/mindvault-v2/sources.json` → `{"sources": [...]}`.
...
| session-end auto compile latency | N/A | <10s | opt-in 단계 (MV2_AUTO_COMPILE 미설정 시 0) |
...
- **install.sh** — MV2_AUTO_COMPILE opt-in 안내 + sources_cli 사용법.
- **GitHub MIT 공개** — 형 결정.
